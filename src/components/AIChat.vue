...
import { sendChatMessageStream, sendPolishRequest, getChatSessions, deleteChatSession, getChatHistory, updateMessagePolishHandled, updateLineEditHandledStatus, type AgentPhaseEvent } from '../api/aiChat';
import type { AIChatMessage, ChatSession } from '../types';
import type { UnlistenFn } from '@tauri-apps/api/event';
import { listen } from '@tauri-apps/api/event';
import { marked } from 'marked';
import { Delete, Close, UserFilled, Cpu, Promotion, Plus, ArrowDown, Check, Close as CloseIcon, QuestionFilled } from '@element-plus/icons-vue';
...
const currentToolCallMsgId = ref<string>(''); // 当前轮次工具消息的ID

// ask_user 输入
const askUserInput = ref(''); // 用户输入的答复
const otherOptionActive = ref(''); // 当前激活"其他"选项的消息ID
      ...
      },
      {
        ...
        onToolCall: (toolName: string, toolDisplayName?: string, toolParams?: Record<string, any>) => {
          ...
          if (!isInToolCallRound.value) {
            ...
            currentToolCallMsgId.value = newMsgId;
          ...
          // 更新当前轮次的工具消息
          const toolMsg = messages.value.find(m => m.id === currentToolCallMsgId.value);
          if (toolMsg) {
            toolMsg.content = JSON.stringify(currentToolCallNames.value);
          }
...
const buildToolDisplayText = (toolName: string, displayName: string, params?: Record<string, any>): string => {
  ...
  switch (toolName) {
    ...
    case 'get_volume_outline': {
      const volTitle = params?.volumeId ? `卷${params.volumeId}` : '卷';
      const typeLabel = params?.outlineType === 'fine' ? '细纲' : '粗纲';
      return `获取${volTitle}${typeLabel}`;
    }
    case 'create_chapter':
      return params?.title ? `创建章节「${params.title}」` : '创建新章节';
    ...
    case 'propose_line_edit': {
      const chTitle = params?.chapterId ? findChapterTitle(params.chapterId) : '章节';
      const lineNum = params?.lineNumber ? `第${params.lineNumber}行` : '';
      return `提议修改${chTitle}${lineNum}`;
    }
    case 'ask_user':
...
// 解析 line_edit 消息数据
const parseLineEditMsg = (msg: AIChatMessage): { chapterId: string; lineNumber: number; originalText: string; newText: string; reason?: string; handled?: boolean } | null => {
  try {
    return JSON.parse(msg.content);
  } catch {
    return null;
...
// 解析 ask_user 消息数据
const parseAskUserMsg = (msg: AIChatMessage): { question: string; context?: string; options?: string[]; handled?: boolean; selectedOption?: string } | null => {
  try {
    return JSON.parse(msg.content);
  } catch {
    return null;
...
// 应用行编辑
const applyLineEdit = async (msg: AIChatMessage) => {
  const data = parseLineEditMsg(msg);
  if (!data || !bookStore.currentBook) return;

  // 发送事件到 WritingArea 进行实际修改
  window.dispatchEvent(new CustomEvent('apply-line-edit', {
    detail: {
      chapterId: data.chapterId,
  ...
  // 标记为已处理
  msg.content = JSON.stringify({ ...data, handled: true });

  // 保存到数据库
  try {
    await updateLineEditHandledStatus(msg.id, 'accepted');
  } catch (e) {
    console.error('保存行编辑状态失败:', e);
  }
...
// 取消行编辑
const cancelLineEdit = async (msg: AIChatMessage) => {
  const data = parseLineEditMsg(msg);
  if (!data) return;
  ...
  // 保存到数据库
  try {
    await updateLineEditHandledStatus(msg.id, 'rejected');
  } catch (e) {
    console.error('保存行编辑状态失败:', e);
  }
...
// 检查行编辑是否已处理
const isLineEditHandled = (msg: AIChatMessage): boolean => {
  try {
    const data = JSON.parse(msg.content);
    return data.handled === true || data.applied === true;
  } catch {
    return false;
  }
};
...
// 检查 ask_user 是否已处理
const isAskUserHandled = (msg: AIChatMessage): boolean => {
  if (msg.handledStatus === 'handled') return true;
  try {
    const data = JSON.parse(msg.content);
...
// 切换"其他"选项的输入框
const toggleOtherOption = (msg: AIChatMessage) => {
  if (otherOptionActive.value === msg.id) {
    otherOptionActive.value = '';
    askUserInput.value = '';
  } else {
    otherOptionActive.value = msg.id;
    askUserInput.value = '';
  }
};

// 处理用户选择 ask_user 选项
const handleAskUserSelect = async (msg: AIChatMessage, option: string) => {
  const data = parseAskUserMsg(msg);
  if (!data) return;

  // 获取 bookId
  const bookId = msg.bookId || bookStore.currentBook?.id;
  if (!bookId) {
    ElMessage.error('无法确定书籍ID，请重新打开对话');
    return;
  }
  if (!bookStore.currentBook || bookStore.currentBook.id !== bookId) {
    ElMessage.error('请先打开对应的书籍');
    return;
  }

  // 标记为已处理
  msg.content = JSON.stringify({ ...data, handled: true, selectedOption: option });
  msg.handledStatus = 'handled';

  // 持久化处理状态
  try {
    await updateLineEditHandledStatus(msg.id, 'handled');
  } catch (e) {
    console.error('保存 ask_user 处理状态失败:', e);
  }

  // 通过 handleSend 流程发送答复，确保流式监听器被正确管理
  isLoading.value = false;
  isStreaming.value = false;
  inputMessage.value = `我选择：${option}`;
  await handleSend();
};

// 处理用户输入 ask_user 答复
const handleAskUserInput = async (msg: AIChatMessage) => {
  if (!askUserInput.value.trim()) return;

  const data = parseAskUserMsg(msg);
  if (!data) return;

  const input = askUserInput.value.trim();
  ...
  // 标记为已处理
  msg.content = JSON.stringify({ ...data, handled: true, selectedOption: input });
  msg.handledStatus = 'handled';
...
let lineEditResultUnlisten: (() => void) | null = null;
let askUserResultUnlisten: (() => void) | null = null;
let fixConflictUnlisten: (() => void) | null = null;
...
onMounted(async () => {
  ...
  // 监听 AI 生成的行编辑结果（来自 function calling 的 propose_line_edit 拦截）
  lineEditResultUnlisten = await listen('ai-line-edit-result', (event) => {
    const data = event.payload as {
      sessionId: string;
      bookId: string;
      messageId: string;
    ...
    const lineEditMsg: AIChatMessage = {
      id: data.messageId,
      bookId: data.bookId,
      chapterId: data.chapterId,
      sessionId: data.sessionId,
      role: 'line_edit' as any,
      content: JSON.stringify({
        chapterId: data.chapterId,
        lineNumber: data.lineNumber,
        originalText: data.originalText,
    ...
    messages.value.push(lineEditMsg);
    scrollToBottom();
  });

  // 监听 AI 生成的 ask_user 结果（来自 function calling 的 ask_user 拦截）
  askUserResultUnlisten = await listen('ai-ask-user-result', (event) => {
    const data = event.payload as {
      sessionId: string;
      bookId: string;
      messageId: string;
    ...
    const askUserMsg: AIChatMessage = {
      id: data.messageId,
      bookId: data.bookId,
      chapterId: '',
      sessionId: data.sessionId,
      role: 'ask_user' as any,
      content: JSON.stringify({
    ...
    messages.value.push(askUserMsg);
    scrollToBottom();
  });

  // 监听设定冲突修复请求（来自 ConflictSidebar 的修复按钮）
  const handleFixConflictEvent = (event: CustomEvent) => {
  ...
  window.addEventListener('ai-fix-conflict', handleFixConflictEvent as EventListener);
  fixConflictUnlisten = () => {
    window.removeEventListener('ai-fix-conflict', handleFixConflictEvent as EventListener);
  };
});
...
onUnmounted(() => {
  ...
  if (lineEditResultUnlisten) {
    lineEditResultUnlisten();
  }
  if (askUserResultUnlisten) {
    askUserResultUnlisten();
  }
  if (fixConflictUnlisten) {
    fixConflictUnlisten();
  }
  polishUnwatch();
});
    ...
    <div ref="messagesContainer" class="messages-container">
      ...
      <template v-else>
        <template v-for="msg in messages" :key="msg.id">
          ...
          <!-- 询问用户消息（来自 function calling 的 ask_user） -->
          <div v-else-if="msg.role === 'ask_user' && parseAskUserMsg(msg)" class="ask-user-card">
            <template v-if="!isAskUserHandled(msg)">
              <div class="ask-user-header">
                <el-icon class="ask-user-icon"><QuestionFilled /></el-icon>
                <span>需要你的选择</span>
              </div>
              <div v-if="parseAskUserMsg(msg)!.context" class="ask-user-context">{{ parseAskUserMsg(msg)!.context }}</div>
              <div class="ask-user-question">{{ parseAskUserMsg(msg)!.question }}</div>
              <div v-if="parseAskUserMsg(msg)!.options && parseAskUserMsg(msg)!.options!.length > 0" class="ask-user-options">
                <div
                  v-for="(option, idx) in parseAskUserMsg(msg)!.options"
                  :key="idx"
                  class="ask-user-option-item"
                  @click="handleAskUserSelect(msg, option)"
                >
                ...
                <!-- 默认的"其他"选项 -->
                <div
                  class="ask-user-option-item ask-user-option-other"
                  :class="{ 'is-active': otherOptionActive === msg.id }"
                  @click="toggleOtherOption(msg)"
                >
                  <span class="option-index">···</span>
                  <span class="option-text">其他（自由填写）</span>
                </div>
                <div v-if="otherOptionActive === msg.id" class="ask-user-other-input">
                  <el-input
                    v-model="askUserInput"
                    placeholder="请输入你的想法..."
                    @keyup.enter="handleAskUserInput(msg)"
                  />
                  <el-button type="primary" size="small" @click="handleAskUserInput(msg)">发送</el-button>
                </div>
              </div>
              <div v-else class="ask-user-input">
                <el-input
                  v-model="askUserInput"
                  placeholder="请输入你的答复..."
                  class="ask-user-free-input"
                  @keyup.enter="handleAskUserInput(msg)"
                />
                <el-button type="primary" @click="handleAskUserInput(msg)">发送</el-button>
              </div>
            </template>
            <template v-else>
              <div class="ask-user-header ask-user-done">
                <el-icon><Check /></el-icon>
                <span>已答复</span>
              </div>
              <div class="ask-user-answered">你的选择：{{ parseAskUserMsg(msg)!.selectedOption || '已处理' }}</div>
            </template>
          </div>
          <!-- 行编辑消息（来自 function calling 的 propose_line_edit） -->
          <div v-else-if="msg.role === 'line_edit' && parseLineEditMsg(msg)" class="line-edit-card">
            <template v-if="!isLineEditHandled(msg)">
              <div class="line-edit-header">提议修改</div>
              <div v-if="parseLineEditMsg(msg)!.reason" class="line-edit-reason">理由：{{ parseLineEditMsg(msg)!.reason }}</div>
              <div class="line-edit-content">
                <div class="line-edit-line">第 {{ parseLineEditMsg(msg)!.lineNumber }} 行</div>
                <div class="line-edit-original">
                  <div class="edit-label">原文：</div>
                  <div class="edit-text">{{ parseLineEditMsg(msg)!.originalText }}</div>
                </div>
                <div class="line-edit-new">
                  <div class="edit-label">修改为：</div>
                  <div class="edit-text">{{ parseLineEditMsg(msg)!.newText }}</div>
                </div>
              </div>
              <div v-if="!historyMessageIds.has(msg.id)" class="line-edit-actions">
                <el-button type="primary" size="small" :icon="Check" @click="applyLineEdit(msg)">
                  应用
                </el-button>
                <el-button size="small" :icon="CloseIcon" @click="cancelLineEdit(msg)" class="cancel-btn">
                  取消
                </el-button>
              </div>
            </template>
            <template v-else>
              <div class="line-edit-header">已处理</div>
              <div class="line-edit-applied">第 {{ parseLineEditMsg(msg)!.lineNumber }} 行修改 — {{ parseLineEditMsg(msg)!.newText }}</div>
            </template>
          </div>
          <!-- 普通消息 -->
...
.ask-user-option-item {
  display: flex;
  align-items: center;
  gap: 10px;
  padding: 10px 14px;
  background: var(--nw-bg-primary);
  border: 1.5px solid var(--nw-border, #e0e0e0);
  border-radius: 8px;
  cursor: pointer;
  transition: all 0.2s ease;
}
.ask-user-option-item:hover {
...
.ask-user-option-other .option-index {
  background: var(--nw-bg-tertiary);
  color: var(--nw-text-secondary);
  font-size: 10px;
  letter-spacing: 2px;
}
.ask-user-option-other.is-active {
  border-color: var(--el-color-primary);
  background: var(--el-color-primary-light-9);
}
.ask-user-option-other.is-active .option-index {
